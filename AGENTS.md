...
- No external dependencies yet (`dependencies = []`). No build backend, and no test/lint/format/typecheck tooling.

## Layout
- `main.py` — entry point; `uv run main.py` prints "Hello from heaven!".
- `pyproject.toml` — project metadata: name `heaven`, version `0.1.0`, readme `README.md`.
- `uv.lock` — lockfile; commit it.
- `.venv/` — virtual environment; gitignored (see `.gitignore`).
- `README.md` — currently just the `# Heaven` heading.
